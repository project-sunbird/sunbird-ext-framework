...
pluginRegistry.initialize({ metaProviderConfig: defaultConfig.db.cassandra });
...
describe('Class PluginRegistry', () => {
  ...
  describe('register method', () => {
    it('should register the plugin with manifest', (done) => {
      ...
      let metaDataProviderStub = Sinon.stub(pluginRegistry.metaDataProvider, 'createMeta').callsFake(() => {
        return new Promise((resolve, reject) => {
          resolve(true);
        })
      })
    ...
    it('should handle error when metaDataProvider throws error', (done) => {
      ...
      let manifest: IPluginManifest = { id: 'test-plugin', name: 'Test Plugin', version: '1.0.0', author: 'sunil A S <sunils@ilimi.in>', server: {} };
      pluginRegistry.register(Manifest.fromJSON(manifest)).catch(() => {
  ...
  describe('isRegistered method', () => {
    it('should return TRUE, if the plugin is registered', (done) => {
      ...
      let metaDataProviderStub = Sinon.stub(pluginRegistry.metaDataProvider, 'getMeta').callsFake(() => {
        return new Promise((resolve, reject) => {
  ...
  describe('updateStatus method', () => {
    it('should update the status of the plugin', (done) => {
      ...
      let metaDataGetMetaStub = Sinon.stub(pluginRegistry.metaDataProvider, 'getMeta').callsFake(() => {
        return new Promise((resolve, reject) => {
          resolve({ rows: [plugin] });
        })
      });
      let metaDataUpdateMetaStub = Sinon.stub(pluginRegistry.metaDataProvider, 'updateMeta').callsFake(() => {
        return new Promise((resolve, reject) => {
          resolve(true);
        });
      });
